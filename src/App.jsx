...
import {
  ...
  Coffee, Crown, Medal, ThumbsUp, Smile, AlertTriangle
} from 'lucide-react';

// --- 0. 错误边界 (Error Boundary) ---
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="fixed inset-0 bg-red-50 flex items-center justify-center p-6 z-[9999]">
          <div className="bg-white p-6 rounded-2xl shadow-xl max-w-md w-full border-2 border-red-100">
            <div className="flex items-center gap-3 text-red-600 mb-4">
              <AlertTriangle className="w-10 h-10" />
              <h2 className="text-xl font-bold">出错了 (Oops!)</h2>
            </div>
            <p className="text-slate-600 mb-4">
              应用遇到了一些问题。这通常是因为浏览器兼容性或临时的内存问题。
            </p>
            <div className="bg-slate-100 p-3 rounded-lg text-xs font-mono text-slate-500 mb-6 overflow-auto max-h-32">
              {this.state.error && this.state.error.toString()}
            </div>
            <button
              onClick={() => window.location.reload()}
              className="w-full bg-red-500 text-white font-bold py-3 rounded-xl shadow-lg active:scale-95 transition"
            >
              重新加载 (Reload)
            </button>
          </div>
...
// [Fix-Mobile] 单例 AudioContext 管理器
...
const playWordAudio = async (word) => {
  ...
  // 确保 AudioContext 是激活的 (针对 iOS/Mobile)
  AudioContextManager.resume();
