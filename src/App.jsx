...
// 更新错题进度 (拼对+1，拼错清零)
// 返回: 'graduated' (毕业), 'improved' (进步), 'reset' (重置)
...
function GameScreen({ words, mode, onBack, isMistakeMode = false }) {
    // 如果是错题本模式，将 Object 转换为 Array
    const workingWords = useMemo(() => {
        if (Array.isArray(words)) return words;
        // 如果传入的是对象（错题本数据），转为数组并打乱
    ...
    const checkAnswer = (finalPlaced) => {
        ...
        if (userPhrase === currentWordObj.word) {
            ...
            if (isMistakeMode) {
                ...
                if (result === 'graduated') {
                    setGraduatedAnimation(true); // 触发毕业动画
                } else {
        ...
        `}>
                    ...
                    `}
                        ...
                        <div className="h-8 mb-6 mt-2 flex items-center justify-center gap-1">
                            {currentWordObj.syllables && currentWordObj.syllables.map((syl, i) => (
                                <React.Fragment key={i}>
                                    <span className={`text-sm md:text-base font-medium transition-colors duration-500 ${isCompleted ? 'text-green-500' : 'text-slate-300'}`}>
                                        {syl}
                                    </span>
                                    {i < currentWordObj.syllables.length - 1 && <span className="text-slate-300 mx-0.5">·</span>}
                                </React.Fragment>
                            ))}
                        </div>
...
export default function App() {
    ...
    const [gameMode, setGameMode] = useState(null); // 'visual', 'blind', 'notebook'
    ...
    useEffect(() => {
        const checkMistakes = () => {
            const db = getMistakes();
            // 修复：轮询时只更新数量，不更新数据对象，防止GameScreen无限重渲染
            // setMistakeData(db);  <-- 删除这一行
    ...
    const startNotebookMode = () => {
        ...
        setMistakeData(db); // 在这里更新数据，保证进入游戏后数据源稳定
            ...
            `}
                    >
                        <div className="flex items-start justify-between mb-4">
                            <div className={`
                w-14 h-14 rounded-2xl flex items-center justify-center text-white shadow-inner
                bg-gradient-to-br from-gray-400/20 to-transparent
                ${unit.themeColor.split(' ')[2].replace('text', 'bg')}
              `}>
                                {React.cloneElement(unit.icon, { className: "w-7 h-7 text-white" })}
                            </div>
