    ...
    if (this.state.hasError) {
      return (
        <div className="flex flex-col items-center justify-center h-screen bg-sky-50 text-center p-6">
          ...
          <button onClick={() => window.location.reload()} className="bg-indigo-500 text-white px-6 py-3 rounded-full font-bold shadow-lg active:scale-95">重新加载</button>
        </div>
      );
    }
...
const DEFAULT_WORDS_DATA = {
  ...
  3: [
      ...
      { word: "jet", cn: "喷气式飞机", emoji: "✈️", syllables: ["jet"] },
      { word: "Kim", cn: "吉姆", emoji: "👦", syllables: ["Kim"] },
      { word: "kite", cn: "风筝", emoji: "🪁", syllables: ["kite"] },
      { word: "leg", cn: "腿", emoji: "🦵", syllables: ["leg"] },
  ],
  4: [
      ...
      { word: "fox", cn: "狐狸", emoji: "🦊", syllables: ["fox"] },
      { word: "pen", cn: "钢笔", emoji: "🖊️", syllables: ["pen"] },
      { word: "cup", cn: "杯子", emoji: "☕", syllables: ["cup"] },
  ],
  5: [
  ...
  6: [
      ...
      { word: "zebra", cn: "斑马", emoji: "🦓", syllables: ["ze", "bra"] },
      { word: "yum", cn: "美味", emoji: "😋", syllables: ["yum"] },
      { word: "water", cn: "水", emoji: "💧", syllables: ["wa", "ter"] },
      { word: "vest", cn: "马甲", emoji: "🦺", syllables: ["vest"] },
      { word: "vat", cn: "大桶", emoji: "🛢️", syllables: ["vat"] },
      { word: "won", cn: "获胜", emoji: "🥇", syllables: ["won"] },
      { word: "yes", cn: "是的", emoji: "✅", syllables: ["yes"] },
      { word: "zig", cn: "锯齿形转角", emoji: "⚡", syllables: ["zig"] },
  ]
};
...
const saveWordsData = (data) => localStorage.setItem(KEYS.WORDS, JSON.stringify(data));
...
const saveMistakes = (data) => localStorage.setItem(KEYS.MISTAKES, JSON.stringify(data));
...
const saveBrawlProgress = (state) => localStorage.setItem(KEYS.BRAWL, JSON.stringify(state));
...
export default function App() {
  ...
  const renderContent = () => {
      ...
      if (gameMode === 'notebook') {
          ...
          return <ErrorBoundary><GameScreen words={words} mode={gameMode} onBack={() => setGameMode(null)} settings={settings} onUpdateStats={handleUpdateStats} /></ErrorBoundary>;
      }
             ...
             <div className="max-w-4xl mx-auto mb-8 px-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                 <div onClick={startNotebookMode} className={`relative bg-white rounded-3xl p-6 shadow-lg border-2 border-red-100 cursor-pointer hover:scale-[1.02] transition flex items-center gap-4 ${dueCount === 0 ? 'opacity-70 grayscale' : ''}`}>
                     ...
                     <div>
                         ...
                         <p className="text-sm text-gray-500">{dueCount > 0 ? `有 ${dueCount} 个单词需要复习` : '暂时没有需要复习的单词'}</p>
                     </div>
                 ...
                 <div onClick={handleBrawlClick} className="relative bg-gradient-to-r from-violet-600 to-indigo-600 rounded-3xl p-6 shadow-lg cursor-pointer hover:scale-[1.02] transition flex items-center gap-4 text-white overflow-hidden">
                     ...
                     <div className="z-10">
                         <h3 className="text-xl font-bold flex items-center gap-2">全明星大乱斗 <span className="bg-yellow-400 text-yellow-900 text-[10px] px-1.5 py-0.5 rounded font-bold">30词</span></h3>
